...
import com.nikita_zayanchkovskij.newsapp.databinding.BusinessNewsFragmentBinding
...
import com.nikita_zayanchkovskij.newsapp.presentation.view_model.news_lists_for_main_screen.BusinessNewsViewModel
import kotlinx.coroutines.flow.collectLatest
import kotlinx.coroutines.launch


class BusinessNewsFragment: Fragment(), ISetDetailedNewsInfoToViewModel {
    private lateinit var binding: BusinessNewsFragmentBinding
    private val businessNewsViewModel: BusinessNewsViewModel by activityViewModels()
    ...
    private fun setUpRecyclerViewAdapter() = with(binding) {
        rcViewBusinessNews.layoutManager = LinearLayoutManager(activity as AppCompatActivity)
        rcViewBusinessNews.adapter = newsAdapter
    }


    private fun receiveNewsData() {
        lifecycleScope.launch {
            businessNewsViewModel.receiveBusinessNews()
        }
    }
    ...
    private fun observeNewsData() {
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                businessNewsViewModel.businessNewsList.collectLatest { newsListState ->
