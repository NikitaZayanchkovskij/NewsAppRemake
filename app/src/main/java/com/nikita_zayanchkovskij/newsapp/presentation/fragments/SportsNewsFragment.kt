...
import com.nikita_zayanchkovskij.newsapp.databinding.SportsNewsFragmentBinding
...
import com.nikita_zayanchkovskij.newsapp.presentation.view_model.news_lists_for_main_screen.SportsNewsViewModel
import kotlinx.coroutines.flow.collectLatest
import kotlinx.coroutines.launch


class SportsNewsFragment: Fragment(), ISetDetailedNewsInfoToViewModel {
    private lateinit var binding: SportsNewsFragmentBinding
    private val sportsNewsViewModel: SportsNewsViewModel by activityViewModels()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        binding = SportsNewsFragmentBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun setUpRecyclerViewAdapter() = with(binding) {
        rcViewSportsNews.layoutManager = LinearLayoutManager(activity as AppCompatActivity)
        rcViewSportsNews.adapter = newsAdapter
    }


    private fun receiveNewsData() {
        lifecycleScope.launch {
            sportsNewsViewModel.receiveSportsNews()
        }
    }
    ...
    private fun observeNewsData() {
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                sportsNewsViewModel.sportsNewsList.collectLatest { newsListState ->
